function InformacionObraAF(a, b) {
    ...
    x = $.ajax({
            url: "Controlador/InformacionObras/InformacionObrasAF/informacionObrasAF.php",
            type: "POST",
            async: true,
            data: datos
        })
        .done(function (dat, est, obj) {
            ...
            if (dat['rowAf'] == 0) {
                ...
                activarModalInformacionObrasAF(dat, b);
...
function activarModalInformacionObrasAF(peticiones, b) {

    var ubi = peticiones[0]['coordenadasPet'].split(",");

    //initializeMap("mapInfoInspeccion", ubi[0], ubi[1], false, 17);

    $("#modalInformacionObra div[class='card-body']").empty();
    $("#modalInformacionObra #imagenesIns").empty();
    $("#modalInformacionObra #imagenesObra").empty();


    $("#modalInformacionObra #CodigosO").text(peticiones[0]['codigo']);
    for (var peticion in peticiones) {
        for (var datos in peticiones[peticion]) {
            ...
            $("#modalInformacionObra div[class='card-body']").append(div);
        }
    }

    if(b == 1){
        VerImg(peticiones[1],'Inspeccion','imagenesIns');
    }if( b == 2){
        $("#imgsO h4").removeClass("text-hide");
        VerImg(peticiones[1],'Inspeccion','imagenesIns');
        VerImg(peticiones[2],'Obra','imagenesObra');
    }

   

    $("#modalInformacionObra #imgsO").hide();
    $("#modalInformacionObra #verimgO").hide();
    $("#modalInformacionObra #infoObra").show().removeClass('animated');
    $("#modalInformacionObra").modal();
    $("#modalInformacionObra #verimgO").show();
    $('#imagenesO').on('click', function () {
        $("#modalInformacionObra #infoObra").hide();
        $("#modalInformacionObra #imgsO").addClass("animated fadeInRight").show();
    });
    $('#InfO').on('click', function () {
        $("#modalInformacionObra #imgsO").hide();
        $("#modalInformacionObra #infoObra").addClass("animated fadeInLeft").show();

    });

}

function VerImg(peticiones,ubi,load){
    for (var peticion in peticiones) {
        for (var img in peticiones[peticion]) {
            var figure = $('<figure class="col-md-4">');
            figure.append(
                $('<a href="Controlador/'+ubi+'/Finalizar/imagenes/' + peticiones[peticion][img] + '" data-size="1600x1067" >').append(
                    $('<img src="Controlador/'+ubi+'/Finalizar/imagenes/' + peticiones[peticion][img] + '" class="img-fluid">')
                )
            );
            $("#modalInformacionObra #" + load).append(figure);
        }
    }
}
