function historialSolicitantePP(tipo) {

    console.log("historialSolicitantePP");
    ...
    x = $.ajax({
            url: "Controlador/Historial/HistorialSolicitantePP/historialSolicitantePP.php",
            type: "POST",
            async: true,
            data: datos
        })
        .done(function (dat, est, obj) {
            ...
            console.log(dat);
            console.log(obj);
            var datosSolicitantePP = dat;
            printSolicitantePP(datosSolicitantePP, tipo);
            if (tipo == 1) {
                activarDataTable('historialSolicitantePubl');
            } else if (tipo == 2) {
                activarDataTable('historialSolicitantePriv');
            }
        ...
        .fail(function (obj, est) {
            console.log("FAIL");
            if (tipo == 1) {
                $("#historialSolicitantePubl .rows").empty();
            } else if (tipo == 2) {
                $("#historialSolicitantePriv .rows").empty();
            }
            status = est;
            console.log(obj);
            console.log("---------------------------------------------------------");
...
function printSolicitantePP(datosSolicitante, tipo) {
    if (tipo == 1) {
        tabla = 'historialSolicitantePubl';
    } else if (tipo == 2) {
        tabla = 'historialSolicitantePriv';
    }
    $("#" + tabla + " .rows").empty();
    var r = 1;
